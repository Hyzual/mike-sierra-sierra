...
get_ip_addr = `$(CONTAINER_RUNTIME) inspect -f '{{.NetworkSettings.Networks.tuleap_default.IPAddress}}' mike-dev`

.PHONY: all dep clean coverage start watch race help
...
build-docker-image: ## Builds the production Docker image
	$(CONTAINER_RUNTIME) build . --file Dockerfile --tag hyzual/mike-sierra-sierra:latest
...
clean-dev-container:
	$(CONTAINER_RUNTIME) container stop mike || true \
	&& $(CONTAINER_RUNTIME) container rm mike || true
...
start: clean-dev-container ## Build and run the dev docker container.
	@$(CONTAINER_RUNTIME) build --file ./tools/docker-dev/Dockerfile.dev --tag hyzual/mike-sierra-sierra:dev ./tools/docker-dev \
	&& $(CONTAINER_RUNTIME) volume create mike_music || true \
	&& $(CONTAINER_RUNTIME) run --detach --publish 8443:8443 --name mike \
		--mount type=bind,source=`pwd`,destination=/app,ro=true \
		...
		--mount type=bind,source=$(GOPATH)/pkg,destination=/go/pkg,ro=true \
		--mount type=volume,source=mike_music,destination=/music,ro=true \
